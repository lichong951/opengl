...
import top.lc951.aircraftattack.Aircraft_Activity;
import top.lc951.aircraftattack.R;
import top.lc951.aircraftattack.archieModel.ArchieForControl;
import top.lc951.aircraftattack.archieModel.BarbetteForDraw;
import top.lc951.aircraftattack.archieModel.BarrelForDraw;
import top.lc951.aircraftattack.arsenal.Arsenal_House;
import top.lc951.aircraftattack.arsenal.House;
import top.lc951.aircraftattack.arsenal.PlaneHouse;
import top.lc951.aircraftattack.commonObject.BallTextureByVertex;
import top.lc951.aircraftattack.commonObject.CubeForDraw;
import top.lc951.aircraftattack.commonObject.DamForDraw;
import top.lc951.aircraftattack.commonObject.DrawBomb;
import top.lc951.aircraftattack.commonObject.LandForm;
import top.lc951.aircraftattack.commonObject.Light_Tower;
import top.lc951.aircraftattack.commonObject.NumberForDraw;
import top.lc951.aircraftattack.commonObject.SkyBall;
import top.lc951.aircraftattack.commonObject.SkyNight;
...
import top.lc951.aircraftattack.core.ShaderManager;
import top.lc951.aircraftattack.menu.MissileMenuForDraw;
import top.lc951.aircraftattack.planeModel.EnemyPlane;
import top.lc951.aircraftattack.planeModel.Plane;
import top.lc951.aircraftattack.tankemodel.MoXingJiaZai;
import top.lc951.aircraftattack.tankemodel.Model;
import top.lc951.aircraftattack.tankemodel.TanKe;

import static top.lc951.aircraftattack.game_view.Constant.*;
...
public class GLGameView extends GLSurfaceView {
    ...
    public GLGameView(Context context)
    {
        ...
        ty=PLANE_Y;//计算摄像机的位置
        tz=Constant.PLANE_Y=MapArray[mapId].length*WIDTH_LALNDFORM/2;
            ...
            switch(actionId)
            {
                ...
                case MotionEvent.ACTION_DOWN:
                    if(isGameMode==1&&isSpecActionState==1)//特别行动----------成功
                    {
                        ...
                        {
                            Toast.makeText(activity, "--------菜单--------------------", Toast.LENGTH_LONG).show();
                            kThread.flag_go=false;//线程标志位
                    ...
                    {
                        //--------------------------战役模式失败了--------或者------视频前的对话框---------------------------------------------
                        if((!isVideo&&isCrash&&isCrashCartoonOver)||(!isVideo&&Constant.is_button_return)||(isVideo&&!isVideoPlaying&&(!isVideoPlaying&&isTrueButtonAction)))
                        {
                        ...
                        //------------------------------赢得本关对话框----------------------------------------------
                        if(Constant.isOvercome&&isCrashCartoonOver)
                        {
                    ...
                    if(isVideo&&!(!isVideoPlaying&&isTrueButtonAction))//如果是视频播放界面
                    {
                        ...
                        if(!is_button_return&&x>680*ratio_width&&y>404*ratio_height)//----进入游戏
                        {
                            ...
                            Constant.PLANE_MOVE_SPAN=15;//飞机的速度
                            PLANE_X=-100;
                            PLANE_Y=300;
                            PLANE_Z=-100;
    ...
    private class SceneRenderer implements GLSurfaceView.Renderer
    {
        ...
        {
            if(Constant.isno_Hit||isno_Vibrate)//如果被击中了
            {
