...
class ColorRect {
    ...
    float u;//基本块在视口上的位置
    float v;
    public ColorRect(MySurfaceView_9 mv)
    {
        //初始化顶点坐标与着色数据
        initVertexData();
        //初始化shader
    ...
    //初始化shader
    public void intShader(MySurfaceView_9 mv)
    {
        //加载顶点着色器的脚本内容
        mVertexShader= ShaderUtil.loadFromAssetsFile("vertex_9.sh", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("frag_9.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
