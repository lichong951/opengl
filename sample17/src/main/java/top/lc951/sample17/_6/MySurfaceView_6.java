...
import top.lc951.sample17.R;

import static top.lc951.sample17._6.Constant.Ceiling_Height;
import static top.lc951.sample17._6.Constant.Stick_Length;
import static top.lc951.sample17._6.Constant.Stick_R;
import static top.lc951.sample17._6.Constant.cubeSize;
...
public class MySurfaceView_6 extends GLSurfaceView {
    ...
    public void initWorld() {
        ...
        stickFBSliderShape = new CylinderShapeZ(new Vector3f(Stick_R, Stick_R, Stick_Length));
        //Z方向圆柱
        stickLRSliderShape = new CylinderShapeX(new Vector3f(Stick_Length, Stick_R, Stick_R));
        //X方向圆柱
        initRigidBody();
        //添加长方体与前后方向的棍之间的约束
        Vector3f originA = new Vector3f(0, 0, 0);
        Vector3f originB = new Vector3f(0, 0, 0);
        addSliderConstraint(0, stickFBSliderBody.body, cubeBody.body, BulletGlobals.SIMD_PI / 2,
                originA, originB, true);//BulletGlobals.SIMD_PI/2
        //添加前后方向的棍与左右方向远端的棍之间的约束
        originA = new Vector3f(0, 0, 0);
        originB = new Vector3f(0, 0, -Stick_Length);
        addSliderConstraint(1, stickLRFSliderBody.body, stickFBSliderBody.body, 0, originA,
                originB, true);
        //添加FB方向的棍与LR方向近端的棍之间的约束
        originA = new Vector3f(0, 0, 0);
        originB = new Vector3f(0, 0, Stick_Length);
        addSliderConstraint(2, stickLRNSliderBody.body, stickFBSliderBody.body, 0, originA,
                originB, true);
    }

    public void initRigidBody() { //TODO
        cubeBody = new RigidBodyHelper(boxShape, 1f, dynamicsWorld, 0.0f, 0.8f, new Vector3f(0,
                Ceiling_Height - 5, 0), true);
        stickFBSliderBody = new RigidBodyHelper(stickFBSliderShape, 1f, dynamicsWorld, 0.0f,
                0.8f, new Vector3f(0, Ceiling_Height - 5, 0), true);
        stickLRFSliderBody = new RigidBodyHelper(stickLRSliderShape, 0, dynamicsWorld, 0.0f,
                0.8f, new Vector3f(0, Ceiling_Height - 5, -Stick_Length), true);
        stickLRNSliderBody = new RigidBodyHelper(stickLRSliderShape, 0, dynamicsWorld, 0.0f,
                0.8f, new Vector3f(0, Ceiling_Height - 5, Stick_Length), true);
    }

    public void addSliderConstraint(int index, RigidBody ra, RigidBody rb, float angle, Vector3f
            originA, Vector3f originB, boolean force) {
    ...
    private class MyRenderer implements GLSurfaceView.Renderer {
        ...
        @Override
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {//TODO
            //设置屏幕背景色RGBA
            ...
            cube = new Cube(MySurfaceView_6.this, cubeSize, cubeBody.body);
            stickFBSliderAxis = new Stick(MySurfaceView_6.this, Stick_Length, Stick_R, 11.25f,
                    new float[]{1, 0, 0, 1},
                    stickFBSliderBody.body);
            stickLRFSliderAxis = new Stick(MySurfaceView_6.this, Stick_Length, Stick_R, 11.25f,
                    new float[]{1, 0, 0, 1},
                    stickLRFSliderBody.body);
            stickLRNSliderAxis = new Stick(MySurfaceView_6.this, Stick_Length, Stick_R, 11.25f,
                    new float[]{1, 0, 0, 1},
                    stickLRNSliderBody.body);
            button = new TexRect(MySurfaceView_6.this, 0.5f, 1f, 1f);
            ...
            new KeyThread(MySurfaceView_6.this).start();
        }
    }
