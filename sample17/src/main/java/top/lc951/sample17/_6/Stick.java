...
class Stick {
    ...
    public Stick(MySurfaceView_6 mv, float length, float circle_radius, float degreespan, float[] colorValue,
                 RigidBody body)
    {
        //初始化顶点坐标与着色数据
        this.mv=mv;
    ...
    {
        //加载顶点着色器的脚本内容
        mVertexShader= ShaderUtil.loadFromAssetsFile("vertex_color_6.sh", mv.getResources());
        ShaderUtil.checkGlError("==ss==");
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("frag_color_6.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
        ShaderUtil.checkGlError("==ss==");
