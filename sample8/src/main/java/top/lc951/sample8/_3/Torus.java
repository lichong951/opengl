...
class Torus {
    ...
    public void initVertexData(
        ...
        for (float angdegCol = 0; Math.ceil(angdegCol) < 360 + angdegColSpan;
            ...
            for (float angdegRow = 0; Math.ceil(angdegRow) < 360 + angdegRowSpan; angdegRow +=
                    angdegRowSpan)//重复了一列顶点，方便了索引的计算
            {
        ...
        for (float angdegCol = 0; Math.ceil(angdegCol) < 360 + angdegColSpan; angdegCol +=
                angdegColSpan) {
            float t = angdegCol / 360;//t坐标
            for (float angdegRow = 0; Math.ceil(angdegRow) < 360 + angdegRowSpan; angdegRow +=
                    angdegRowSpan)//重复了一列纹理坐标，以索引的计算
            {
    ...
    {
        //加载顶点着色器的脚本内容
        mVertexShader= ShaderUtil.loadFromAssetsFile("vertex_tex_3.sh", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader=ShaderUtil.loadFromAssetsFile("frag_tex_3.sh", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
