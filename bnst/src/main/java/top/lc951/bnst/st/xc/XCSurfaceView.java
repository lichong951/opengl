...
import static top.lc951.bnst.st.xc.Constant_xc.HOUSE_GAO;
import static top.lc951.bnst.st.xc.Constant_xc.LENGTH_DISPLAY;
import static top.lc951.bnst.st.xc.Constant_xc.RADIUS_DISPLAY;
import static top.lc951.bnst.st.xc.Constant_xc.RATIO_BOAT;
import static top.lc951.bnst.st.xc.Constant_xc.SCREEN_HEIGHT;
import static top.lc951.bnst.st.xc.Constant_xc.SCREEN_WIDTH;
import static top.lc951.bnst.st.xc.Constant_xc.WALL_WIDHT;
import static top.lc951.bnst.st.xc.Constant_xc.XC_DISTANCE;
import static top.lc951.bnst.st.xc.Constant_xc.XC_Self_Adapter_Data_TRASLATE;
...
public class XCSurfaceView extends GLSurfaceView {
    ...
    public XCSurfaceView(Context context) {
        ...
        cx = (float) (tx + Math.cos(Math.toRadians(xAngle)) * Math.sin(Math.toRadians(yAngle)) *
                XC_DISTANCE);//摄像机x坐标
        cz = (float) (tz + Math.cos(Math.toRadians(xAngle)) * Math.cos(Math.toRadians(yAngle)) *
                XC_DISTANCE);//摄像机z坐标
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        switch (e.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                cx = (float) (tx + Math.cos(Math.toRadians(xAngle)) * Math.sin(Math.toRadians
                        (yAngle)) * XC_DISTANCE);//摄像机x坐标
                cz = (float) (tz + Math.cos(Math.toRadians(xAngle)) * Math.cos(Math.toRadians
                        (yAngle)) * XC_DISTANCE);//摄像机z坐标
                cy = (float) (ty + Math.sin(Math.toRadians(xAngle)) * XC_DISTANCE);//摄像机y坐标
                break;
            ...
            for (int i = 0; i < BoatInfo.boatPartNames[j].length; i++) {
                parts[j][i] = LoadUtilTexXC.loadFromFileVertexOnly
                        (
                                ...
                                r,
                                ShaderManager_xc.getCommTextureShaderProgram()
                        );
            }
        }
    }
    ...
    private class SceneRenderer implements GLSurfaceView.Renderer {
        ...
        public void onDrawFrame(GL10 gl) {
            ...
            MatrixState.translate(0, -HOUSE_GAO / 2, 0);
                    ...
                    while (flagForThread) {
                        try {
                            ...
                            if (flag_go)//如果允许惯性
                            {
                                countGoStep--;
                                if (countGoStep <= 0) {
                                    curr_angle_speed = curr_angle_speed + acceleratedVelocity;
                                    //计算当前角速度
                                }

                                if (Math.abs(curr_angle_speed) > 0.1f) {
                                    yAngle = yAngle + curr_angle_speed;
                                    cx = (float) (tx + Math.cos(Math.toRadians(xAngle)) * Math
                                            .sin(Math.toRadians(yAngle)) * XC_DISTANCE);//摄像机x坐标
                                    cz = (float) (tz + Math.cos(Math.toRadians(xAngle)) * Math
                                            .cos(Math.toRadians(yAngle)) * XC_DISTANCE);//摄像机z坐标
                                    cy = (float) (ty + Math.sin(Math.toRadians(xAngle)) *
                                            XC_DISTANCE);//摄像机y坐标
                                } else {
                                    curr_angle_speed = 0;
                                    flag_go = false;
        ...
        public void onSurfaceCreated(GL10 gl, EGLConfig config) {
            synchronized (MyActivity.boatInitLock) {
                ShaderManager_xc.compileShaderReal();
                ...
                button = new TextureRect(ShaderManager_xc.getCommTextureShaderProgram(),
                        half_width_button, XC_Self_Adapter_Data_TRASLATE[Constant_clp
                        .screenId][0]);//绘制相关按钮

                //创建罗马柱对象
                romeData.initShader(ShaderManager_xc.getColorshaderProgram());
                rome = romeData;
                //开启背面剪裁
        ...
        public void drawBoatMirrorOnWall() {
            ...
            final int span = 68;
            //在第一面墙
            if (yAngleTemp <= span && yAngleTemp >= 0 || yAngleTemp >= 360 - span && yAngleTemp
                    <= 360) {
                MatrixState.pushMatrix();
    ...
    public void virtualButton() {
        ...
        float topEdge = (float) (1 - XC_Self_Adapter_Data_TRASLATE[Constant_clp
                .screenId][0] - offset_Y_Button1) / 2 * SCREEN_HEIGHT;
        float bottomEdge = (float) (1 + XC_Self_Adapter_Data_TRASLATE[Constant_clp
                .screenId][0] - offset_Y_Button1) / 2 * SCREEN_HEIGHT;
        ...
        topEdge = (float) (1 - XC_Self_Adapter_Data_TRASLATE[Constant_clp
                .screenId][0] - offset_Y_Button2) / 2 * SCREEN_HEIGHT;
        bottomEdge = (float) (1 + XC_Self_Adapter_Data_TRASLATE[Constant_clp
                .screenId][0] - offset_Y_Button2) / 2 * SCREEN_HEIGHT;
