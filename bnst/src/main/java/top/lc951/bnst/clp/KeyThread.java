...
import top.lc951.bnst.st.d2.DBUtil;
import top.lc951.bnst.st.d2.MyActivity;

import static top.lc951.bnst.clp.Constant_clp.BOAT_A;
import static top.lc951.bnst.clp.Constant_clp.BOAT_UNIT_SIZE;
import static top.lc951.bnst.clp.Constant_clp.CURR_BOAT_V;
import static top.lc951.bnst.clp.Constant_clp.CURR_BOAT_V_PZ;
import static top.lc951.bnst.clp.Constant_clp.CURR_BOAT_V_TMD;
import static top.lc951.bnst.clp.Constant_clp.DEGREE_SPAN;
import static top.lc951.bnst.clp.Constant_clp.DISTANCE;
import static top.lc951.bnst.clp.Constant_clp.MAP_ARRAY;
import static top.lc951.bnst.clp.Constant_clp.Max_BOAT_V;
import static top.lc951.bnst.clp.Constant_clp.Max_BOAT_V_VALUE;
import static top.lc951.bnst.clp.Constant_clp.RANK_FOR_HELP;
import static top.lc951.bnst.clp.Constant_clp.RANK_FOR_HERO_BOAT;
import static top.lc951.bnst.clp.Constant_clp.SoundEffectFlag;
import static top.lc951.bnst.clp.Constant_clp.UNIT_SIZE;
import static top.lc951.bnst.clp.Constant_clp.halfFlag;
...
import static top.lc951.bnst.clp.MyGLSurfaceView.bx;
import static top.lc951.bnst.clp.MyGLSurfaceView.bz;
import static top.lc951.bnst.clp.MyGLSurfaceView.cx;
import static top.lc951.bnst.clp.MyGLSurfaceView.cz;
import static top.lc951.bnst.clp.MyGLSurfaceView.sight_angle;
import static top.lc951.bnst.clp.MyGLSurfaceView.yachtLeftOrRightAngle;
import static top.lc951.bnst.clp.MyGLSurfaceView.yachtLeftOrRightAngleA;
import static top.lc951.bnst.clp.MyGLSurfaceView.yachtLeftOrRightAngleMax;
...
//					{
                    ...
                    //设置新的观察目标点XZ坐标
                    MyGLSurfaceView.tx=(float)(cx-Math.sin(Math.toRadians(sight_angle-yachtLeftOrRightAngle/2))*DISTANCE);//观察目标点x坐标
                    MyGLSurfaceView.tz=(float)(cz-Math.cos(Math.toRadians(sight_angle-yachtLeftOrRightAngle/2))*DISTANCE);//观察目标点z坐标
//					}
                }
            }
                ...
                for(int i=0;i<mgmv.otherPaths.size();i++)
                {
                    ...
                    {//若是大步调整
                        ...
                        if(distance<1)
                        {
                            ...
                            {
                                stepIndex[i]=0;
                                Constant_clp.BOAT_LAP_NUMBER_OTHER[i]= Constant_clp.BOAT_LAP_NUMBER_OTHER[i]+1;
                                //若圈数到了
                                if(Constant_clp.BOAT_LAP_NUMBER_OTHER[i]==3)
                                {
                                    stepStatus[i]=2;
                                    RANK_FOR_HELP= RANK_FOR_HELP+1;
                                }
                            }
                        }
                        else
                        {
                            bdbzxb[i]=(int) (distance/ Constant_clp.Max_BOAT_V_OTHER[i]);
        ...
        if(dis<=60&&bOldZ>RACE_BEGIN_Z&&carTempZ<=RACE_BEGIN_Z)
        {
            if(halfFlag==true)
            {
                ...
                if(numberOfTurns==3&&isSpeedMode)	//若为竞速模式，在跑完圈数后，获得当前的名次，并且把速度和加速度设置为0
                {
                    ...
                    CURR_BOAT_V=0;
                    BOAT_A=0;
                    String currSysTime= Constant_clp.getCurrTime();
                    String currUseTime= Constant_clp.getUseTime();
                ...
                else if(numberOfTurns==3&&!isSpeedMode)		//若为计时模式
                {
                    ...
                    Constant_clp.isBreakRecord=isFast(currUseTime,alist);
